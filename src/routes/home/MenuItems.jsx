...
function MenuItems() {
    ...
    const splicedMenuItems = meals.slice(10, 16);

    const cartCtx = useContext(CartContext)

    const handleAddMealToCart = () => {
        cartCtx.addItem(splicedMenuItems)
    }
        ...
        <div className=' py-10 max-lg:py-5 flex flex-wrap gap-3'>
            {
                isLoading ?
                    fillNum.map((____, index) => {
                        return (
                            <div key={index} className='w-[32%] max-[850px]:w-[49%] max-sm:w-[100%] h-[40vh]'>
                        ...
                        return (
                            <div
                                ...
                                <main className='flex flex-col justify-center items-center p-3'>
                                    ...
                                    <p className='bg-[rgba(255,231,0,0.1)] px-4 py-1 text-[rgba(255,231,0,0.9)] font-medium rounded mb-3 text-[13px] max-md:text-[15px]'>{currencyFormatter.format(meal.price)}</p>
                                    <p className='text-center text-[rgba(6, 6, 5, 0.9)]'>{meal.dsc}</p>
                                </main>
                                <Button onClick={handleAddMealToCart} myStyle='block mx-auto bg-[#FFE700] text-stone-900 px-4 py-2 text-[16px] rounded max-md:text-[12px]'>Add To Cart</Button>
                            </div>
                        )
                    })
            }
